...
(function($) {
	$.fn.styler = function(opt) {
		...
		return this.each(function() {
			...
			// checkbox
			if (el.is(':checkbox')) {
				el.css({position: 'absolute', top: -9999}).each(function() {
			...
			// radio bol
			} else if (el.is(':radio.bol')) {
				el.css({position: 'absolute', left: -9999}).each(function() {
					if (el.next('span.jq-radio-bol').length < 1) {
						var radio = $('<span' + id + ' class="jq-radio-bol' + cl + '" style="display: inline-block"><span></span></span>');
						el.after(radio);
			...
			// file
				...
				el.css({position: 'absolute', top: '-50%', right: '-50%', fontSize: '200px', opacity: 0}).each(function() {
					if (el.parent('span.jq-file').length < 1) {
						...
						var name = $('<div class="name" style="float: left; white-space: nowrap">' + opt.nameText + '</div>').appendTo(file);
						el.after(file);
						file.append(el);
						if (el.is(':disabled')) file.addClass('disabled');
						el.change(function() {
							name.text(el.val().replace(/.+[\\\/]/, '')).addClass('icon');
						})
						.focus(function() {
